...
if (isset($_GET['id_denda'])) {
    ...
    $queryGetDenda = "SELECT * FROM denda WHERE id_denda = '$id'";
    ...
    if ($resultGetDenda->num_rows > 0) {
        ...
        $queryDelete = "DELETE FROM denda WHERE id_denda = '$id'";
        ...
        if ($conn->query($queryDelete) === TRUE) {
            ...
            $message = "Success!";
            echo "<script>alert('$message'); window.location.href = 'laporan-denda.php';</script>";
            exit();
        } else {
        ...
        header('Location: laporan-denda.php');
        exit();
    }
