...
try {
    $end_date_filter_str = date('Y-m-t', strtotime("$tahun-$bulan-01"));
    $end_date_filter_dt = new DateTime($end_date_filter_str);
    $start_date_filter_dt = new DateTime("$tahun-$bulan-01");
} catch (Exception $e) {
    $bulan = date('m');
    $tahun = date('Y');
    $end_date_filter_str = date('Y-m-t', strtotime("$tahun-$bulan-01"));
...
$is_locked = isDataLocked($conn, $bulan, $tahun);
...
$stmt_rincian->bind_param("ss", $bulan, $tahun);
...
$stmt_holidays->bind_param("s", $tahun);
...
foreach ($karyawan_list as $karyawan) {
    ...
    $stmt_cb->bind_param("sss", $nip_karyawan, $bulan, $tahun);
    $stmt_cb->execute();
            ...
            <div class="container-fluid px-lg-4">
                        ...
                        <div class="row g-3 align-items-center">
                            ...
                            <div class="col-lg-6 text-lg-end">
                                ...
                                <div class="btn-group" role="group">
                                    <button id="generate" class="btn btn-success" onclick="generateDataAndCashbon()" <?php if ($is_locked) echo 'disabled'; ?>><i class="fa-solid fa-list-check"></i> Generate Data</button>
                                    <button id="lock" class="btn btn-warning" onclick="lockData()" <?php if ($is_locked) echo 'disabled'; ?>><i class="fas fa-lock"></i> Lock Data</button>
                                    <button id="unlock" class="btn btn-secondary" onclick="unlockData()" <?php if (!$is_locked) echo 'disabled'; ?>><i class="fas fa-lock-open"></i> Unlock Data</button>
                                    <div class="btn-group" role="group">
                                        <button class="btn btn-dark dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="fas fa-download"></i> Simpan</button>
                                        <ul class="dropdown-menu">
                                            <li><a class="dropdown-item" href="#" onclick="exportTableToExcel('tabel-gaji', 'laporan-gaji-<?php echo $bulan . '-' . $tahun; ?>')"><i class="fas fa-file-excel me-2"></i> Simpan ke Excel (.xls)</a></li>
                                            <li><a class="dropdown-item" href="#" onclick="exportTableToCSV('laporan-gaji-<?php echo $bulan . '-' . $tahun; ?>.csv')"><i class="fas fa-file-csv me-2"></i> Simpan ke CSV</a></li>
                ...
                <div class="card shadow-sm">
                        <div class="card-header">
                            <h5 class="mb-0">Laporan Gaji Karyawan - Periode <?php echo $bulanNames[$bulan] . ' ' . $tahun; ?></h5>
                        </div>
                        <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover table-sm mb-0" id="tabel-gaji">
                                            ...
                                            if (empty($rincian_gaji_map)) {
                                                ...
                                                foreach ($karyawan_list as $karyawan) {
                                                    ...
                                                    if (isset($rincian_gaji_map[$nip])) {
                                                        ...
                                                        $total_tunjangan_prorate = ($karyawan['tunjangan'] ?? 0) + $tunjangan_masa_kerja;
                                                        ...
                                                        $start_date_bulan_ini_str = "$tahun-$bulan-01";
                                                        $end_date_bulan_ini_str = $end_date_filter_str;
                                                        ...
                                                        $stmt_cuti_bulan->bind_param("sss", $nip, $start_date_bulan_ini_str, $end_date_bulan_ini_str);
                                                        $stmt_cuti_bulan->execute();
                                                        $result_cuti_bulan = $stmt_cuti_bulan->get_result();

                                                        while ($cuti_row_bulan = $result_cuti_bulan->fetch_assoc()) {
                                                            $cuti_start_bulan = new DateTime(max($cuti_row_bulan['tgl_mulai'], $start_date_bulan_ini_str));
                                                            $cuti_end_bulan = new DateTime(min($cuti_row_bulan['tgl_selesai'], $end_date_bulan_ini_str));
                                                        ...
                                                        $total_gaji_untuk_denda = $karyawan['gaji_pokok'] + $total_tunjangan_prorate;
                                                ...
                                                >
                                                    ...
                                                    <td class="no-print text-center">
                                                        <a href="slip-gaji.php?nip=<?php echo $nip; ?>&bulan=<?php echo $bulan; ?>&tahun=<?php echo $tahun; ?>" class="btn btn-info btn-sm" target="_blank" title="Lihat Detail"><i class="fa-solid fa-eye"></i></a>
    ...
    <script>
        function deleteGaji(id_rincian_gaji) {
            if (confirm("Apakah Anda yakin ingin menghapus data gaji ini?")) {
                window.location.href = "penggajian.php?deleteID=" + id_rincian_gaji + "&bulan=<?php echo $bulan; ?>&tahun=<?php echo $tahun; ?>";
            }
        }
