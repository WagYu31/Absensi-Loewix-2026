...
// Cek apakah pengguna telah login dan memiliki peran sebagai admin atau superadmin
...
include '../conn.php';
include 'get-kar-login-data.php'; // Menyediakan info admin yang login
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['simpan_cuti_manual'])) {
    ...
    if (empty($nip_karyawan) || empty($tgl_mulai) || empty($tgl_selesai) || empty($jenis_cuti) || empty($keterangan)) {
        ...
        // Handle upload file bukti jika ada (mirip dengan form karyawan)
        if (isset($_FILES['bukti']) && $_FILES['bukti']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_ext, $extensions_allowed) && $file_size <= $max_file_size) {
                // Gunakan NIP karyawan yang dipilih untuk nama file, bukan NIP admin
        ...
        // Lanjutkan insert ke database jika tidak ada error
        if (empty($pesan_error)) {
            // Perhatikan perbedaannya: status 'Disetujui' dan ada kolom `potong_gaji`
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location: " . $_SERVER['PHP_SELF']); // Redirect ke halaman ini lagi (PRG Pattern)
                    exit();
                } else {
...
<body>
    ...
    $(document).ready(function() {
        // Inisialisasi Select2 untuk dropdown karyawan agar bisa dicari
        ...
        $(document).ready(function() {
            ...
            $('.sidebar-menu a').each(function() {
                ...
                if (linkHref === currentPath) {
                    ...
                    // $(this).removeClass('active'); // Biarkan agar tidak menghapus jika ada parameter
                }
            });
