...
if (isset($_GET['id'])) {
    ...
    $queryGetDenda = "SELECT * FROM absen WHERE id = '$id'";
    ...
    if ($resultGetDenda->num_rows > 0) {
        ...
        $queryDelete = "DELETE FROM absen WHERE id = '$id'";

        if ($conn->query($queryDelete) === TRUE) {
            // Redirect ke halaman denda-karyawan.php jika berhasil dihapus
            $message = "Success!";
            echo "<script>alert('$message'); window.location.href = 'request_jam.php';</script>";
            exit();
        } else {
        ...
        header('Location: request_jam.php');
        exit();
    }
