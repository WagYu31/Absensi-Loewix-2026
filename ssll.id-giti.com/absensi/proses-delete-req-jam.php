...
if (isset($_GET['id'])) {
    ...
    $queryGetDenda = "SELECT * FROM izin_jam_kerja WHERE id = '$id'";
    ...
    if ($resultGetDenda->num_rows > 0) {
        ...
        $queryDelete = "DELETE FROM izin_jam_kerja WHERE id = '$id'";

        if ($conn->query($queryDelete) === TRUE) {
            // Redirect ke halaman denda-karyawan.php jika berhasil dihapus
            $message = "Success!";
            echo "<script>alert('$message'); window.location.href = 'req_jam_kerja.php';</script>";
            exit();
        } else {
        ...
        header('Location: req_jam_kerja.php');
        exit();
    }
